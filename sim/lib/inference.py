...
def make_bayes_opt_functions(args):
    ...
    def eval_objective(param):
        new_G, new_G_sem = composite_simulation(param.squeeze())
        return new_G, new_G_sem
...
#
#     # DE parameters
#     de_mutation = args.mutation
#     de_crossover = args.crossover
#     de_strategy = args.strategy
#     de_pop_size = args.pop_size
#     de_type = args.de_type  # type of update
#
#     """
#     Bayesian optimization pipeline
#     """
...
#
#     lockdown_start_date = pd.to_datetime(
#         settings_lockdown_dates[data_country]['start'])
#         # settings_lockdown_dates[args.country]['start'])
#     lockdown_end_date = pd.to_datetime(
#         settings_lockdown_dates[data_country]['end'])
#         # settings_lockdown_dates[args.country]['end'])
#
#     days_until_lockdown_start = (lockdown_start_date - sim_start_date).days
#     days_until_lockdown_end = (lockdown_end_date - sim_start_date).days
...
#         '''
#         Objective function
#         Note: in BO, objectives are maximized; in DE, objectives are minimized
#         '''
#         return (G - G_obs).pow(2).sum(dim=-1)
#         # return - (G - G_obs).pow(2).sum(dim=-1)
#
